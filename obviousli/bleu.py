...
class BLEU(object):
    ...
    @staticmethod
    def compute(candidate, references, weights=(0.25, 0.25, 0.25, 0.25)):
...
def test_BLEU():
    ...
    assert abs(BLEU.modified_precision(
        candidate1,
        [reference1, reference2, reference3],
        n=1
        ) - 0.94) < 1e-2
...
def test_BLEU_penalty():
    ...
    assert abs(BLEU.compute(candidate1, [reference1, reference2, reference3], weights) - 0.504) < 1e-3
    assert abs(BLEU.compute(candidate2, [reference1, reference2, reference3], weights) - 0.457) < 1e-3
